package com.emake.passport.common.utils;
...
public class CookieHelper {
     ...
     * @return Cookie
     */
    public static Cookie findCookieByName(HttpServletRequest request, String domain,String cookieName) {
        Cookie[] cookies = request.getCookies();
        if (cookies == null)
            return null;
        for (int i = 0; i < cookies.length; i++) {
            if (cookies[i].getName().equals(cookieName)&&cookies[i].getDomain().equals(domain)) {
                return cookies[i];
            }
        }
        return null;
